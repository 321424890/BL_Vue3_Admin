	...
	<div class="searchHeader" :style="{ height: computedHeight }">
		<el-form ref="formRef" :model="formData" :inline="true" :rules="formRules" class="form-container">
			<template v-for="(queryItem, index) in displayedQueryConfig" :key="index">
				<el-form-item v-if="queryItem.label" :label="queryItem.label" :prop="queryItem.prop" :required="queryItem.required" class="form-item">
					<el-input
					...
					>
						<el-option v-for="option in queryItem.props.options" :key="option.value" :label="option.label" :value="option.value" />
					</el-select>
					<el-date-picker
